...
from app.clients.ado import download_attachment, extract_image_attachments
...
def _process_one(
    ...
    # Extract and describe image attachments — appended before LLM call
    images = extract_image_attachments(raw_item)
    if images:
        image_text = describe_images(images, download_attachment, source_type="ADO work item")
        if image_text:
            serialised = serialised + "\n\n" + image_text
            log.info("  %d image(s) described and appended for #%d", len(images), item.id)
