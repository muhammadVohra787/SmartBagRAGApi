...
from app.clients.graph import download_image, extract_image_attachments as graph_extract_images
...
def _process_one(
    ...
    # -- Extract images from all messages and append descriptions -------------
    all_images: list[dict] = []
    for msg in thread["raw_messages"]:
        all_images.extend(graph_extract_images(msg))

    if all_images:
        image_text = describe_images(all_images, download_image, source_type="Teams thread")
        if image_text:
            flat = flat + "\n\n" + image_text
            log.info("  %d image(s) described and appended for thread %s", len(all_images), thread_id)
